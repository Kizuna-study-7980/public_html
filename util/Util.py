...
def getButtonHTML(type, name, value, display_value, link_url):
    ...
    button_html = html_type + html_name + html_value + html_onclick + ">" + html_display_value + "</button>"

    return button_html


def addPostForm(String):
    return '<form method="post">' + String + "</form>"
...
def getTextBoxHTML(type, name, value, maxlength, size):
    ...
    button_html = html_type + html_name + html_value + html_maxlength + html_size + ">"

    return button_html


def getLinkRoot():
    return "<a href='http://tojikaji.xsrv.jp/'>Home</a>"
